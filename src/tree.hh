...
class tree {
	...
	public:
      ...
      leaf_iterator   end_leaf(const iterator_base& top) const;
		...
		/// print_unique_roots content of a single-root tree
		void print_unique_root(pre_order_iterator start, std::string indent, bool last);
		/// print_unique_roots content of a multi-root tree
...
template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::print(std::string indent, bool last)
	{
		sibling_iterator sib=begin();
		while(sib!=end()) 
		{
			print_unique_root(pre_order_iterator(sib), "", true);
			++sib;
		}
	}

template <class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::print_unique_root(pre_order_iterator start, std::string indent, bool last)
	{
		std::cout << indent + "+- " << start.node->data << std::endl;
    	indent += last ? "   " : "|  ";

		for(int i=0; i < number_of_children(start); i++)
		{
			pre_order_iterator it_l = child(start, i);
			print_unique_root(it_l, indent, i == number_of_children(start) - 1);
		}
	}
